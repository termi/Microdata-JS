 ...
 * @date: 10.08.2011
 *
 * Implementation of the HTMl5 Microdata specification for IE < 8
 *
...
/** @define {boolean} */
var __GCC__IELT8_USE_HTC__ = true;
// [[[|||---=== GCC DEFINES END ===---|||]]]

;(function(global) {

if("getItems" in document)return;//Test for executing this script in non-IElt8 browser
...
if((tmp = window["Node"]) && (tmp = tmp.prototype) && tmp["ielt8"]) {//IE < 8 polifill
	...
	var
	  ...
	  , get__DOMStringCollection_property = function(thisObj, attributeName) {
			...
			var value = thisObj.getAttribute(attributeName, 1),
			...
			if(!_currentValue) {
				_currentValue = _[attributeName] = new DOMStringCollection_(function() {
					return thisObj.getAttribute(attributeName, 1);
				}, function() {
					thisObj.setAttribute(attributeName, this + "", 1);
				}, thisObj);
			}
	...
	_similar_Object_defineProperties_ = function(element_proto, props) {
		...
		for(var i in props) {
			if(Object.prototype.hasOwnProperty.call(props, i)) {
				prop = props[i];
				if("get" in prop)element_proto["get" + i] = prop["get"];
				if("set" in prop)element_proto["set" + i] = prop["set"];
			}
		}
	};


	_similar_Object_defineProperties_(global["Element"].prototype, {
		...
		"itemProp" : {
			...
			"set" : function(val) {
				return this.setAttribute("itemprop", val + "", 1), val
			}
		},
