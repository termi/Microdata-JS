...
var MicrodataJS = global["MicrodataJS"] = {
	...
	"setItemValue" : function(element, value) {
		...
		if(element.getAttribute("itemprop") === null) {
			//TODO:: writing to itemValue must throw an INVALID_ACCESS_ERR error if the element does not have an itemprop attribute
			return null;
		}
	...
	/**
	 * [getter / setter]
	 * Torn On/Off live Element.properties prop update for browsers without Microdata support
	 * @param {boolean} val true - turn on, false - turn off
	 */
	"liveProperties" : function(val) {
		return val !== undefined ? MicrodataJS["getItemProperties"].force = val : MicrodataJS["getItemProperties"].force;
	}
	,
	/**
	 * @type {Array.<Function>}
...
// 1. Microdata unsupported
	...
	// --- === HTMLPropertiesCollection CLASS [END] === ---
		 ...
		 *
		 * @param {Element} item The item for which to find the properties.
		 * @param {boolean} force Force update "properties" cache
		 * @return {HTMLPropertiesCollection} The properties for the item.
		 */
		var getProperties = MicrodataJS["getItemProperties"] = function(item, force) {
			...
			if(properties) {
				if(!force && !getProperties.force)return properties;
		...
		return function(_element, force) {//return MicrodataJS.fixItemElement
			var val;
			if(_element['itemScope'] !== true)_element['itemScope'] = true;
			...
			// Set getter "properties" with cache to the element
			function _getter_getProperties() {
				return getProperties(this, force);
			}
			if(_element.__defineGetter__) {
				if(!_element.__lookupGetter__('properties'))
					_element.__defineGetter__('properties', _getter_getProperties)
			}
			else if(Object.defineProperty) {
				if(!("properties" in _element))
					Object.defineProperty(_element, "properties", {
						get : _getter_getProperties
					});
			}
			...
			//update cache
			force && getProperties(_element, force);
